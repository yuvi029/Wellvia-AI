        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((service, index) => (
            <Link
              to={service.link}
              key={index}
              className={`${service.bg} p-8 rounded-2xl shadow-md hover:shadow-xl transform hover:-translate-y-2 transition-all duration-300 text-left cursor-pointer block`}
            >
              ...
              <span className="font-medium flex items-center gap-1 text-teal-600 hover:text-teal-400 dark:text-teal-400 dark:hover:text-teal-200">
                Learn More →
              </span>
            </Link>
          ))}
